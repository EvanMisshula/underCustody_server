...
export default class SortableTable extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (
      state.xAttribute !== props.xAttribute ||
      state.yAttribute !== props.yAttribute
    ) {
      return {
        data: props.data,
        xAttribute: props.xAttribute,
  ...
  onClick(event) {
    ...
    newdata.sort(function (a, b) {
      return parseFloat(a[col].replace(/,/g, "")) ===
        parseFloat(b[col].replace(/,/g, ""))
        ? 0
        : parseFloat(a[col].replace(/,/g, "")) >
          parseFloat(b[col].replace(/,/g, ""))
        ? asc
        : -1 * asc;
    });
